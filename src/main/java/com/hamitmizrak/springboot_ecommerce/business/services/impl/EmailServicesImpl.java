...
import com.hamitmizrak.springboot_ecommerce.business.services.IEmailServices;
...
public class EmailServicesImpl implements IEmailServices<EmailDto, EmailEntity> {
    ...
    public EmailDto entityToDto(EmailEntity emailEntity) {
        return modelMapperBeanClass.getModelMapperBeanMethod().map(emailEntity, EmailDto.class);
    }

    @Override
    public EmailEntity dtoToEntity(EmailDto emailDto) {
        return modelMapperBeanClass.getModelMapperBeanMethod().map(emailDto, EmailEntity.class);
    }
